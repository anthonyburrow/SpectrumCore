...
def read(filename: str) -> np.ndarray:
    if pathlib.Path(filename).suffix == '.fits':
        try:
...
def _load_fits(filename: str) -> np.ndarray:
...
def read_sp_data_fits(filename: str) -> np.ndarray:
...
def _load_other(filename: str) -> np.ndarray:
